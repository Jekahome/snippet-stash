...
Числа хранятся с ограниченной точностью, и после каждой операции результат округляется.

Из-за этого при **параллельном** суммировании:

* данные разбиваются на части;
* каждая часть суммируется отдельно;
* потом частичные суммы объединяются;

И порядок операций меняется, поэтому результат может немного отличаться от последовательного вычисления.
