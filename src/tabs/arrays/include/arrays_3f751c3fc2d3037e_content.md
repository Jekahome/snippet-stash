...
Реализация Stack заданного размера через `Vec<T>` имеет ненужную избыточность.
Нет смысла использовать `Vec<T>` с переменной длинной, который, к тому же, располагает значения на куче.
Используйте массив [T;N] фиксированной длины.
Это же позволит располагать Stack и на стеке.
